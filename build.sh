...
# Odin style/vet flags (per Odin book snippet)
DO_STRICT_STYLE=1
DO_VET_STYLE=1

# Extra Odin flags (optional)
EXTRA_FLAGS=()
...
Examples:
  ./build.sh --build
  ./build.sh --run
  ./build.sh --clean --build --run --release
  ./build.sh --build -- --sanitize:address
EOF
}
