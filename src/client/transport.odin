...
try_tcp_connect :: proc(t: ^Transport, timeout_ms: u32) -> bool {
    ...
    // Attempt connect
    conn_err := net.connect(sock, t.server_addr)
    if conn_err != nil && conn_err != .In_Progress {
        net.close(sock)
        return false
    }

    // Wait for connection (simplified - poll would be better)
    // For now, just set blocking and hope for the best
...
setup_udp :: proc(t: ^Transport) -> bool {
    ...
    t.udp_socket = net.UDP_Socket(sock)
    t.type = .UDP
    t.state = .Connected
    return true
}
...
transport_disconnect :: proc(t: ^Transport) {
    if t.state != .Disconnected {
        if t.type == .TCP {
            ...
            net.close(net.any_socket(t.udp_socket))
        }
    }
    t.state = .Disconnected
}
...
transport_send :: proc(t: ^Transport, data: []u8) -> bool {
    ...
    if t.type == .TCP {
        ...
        for total_sent < len(framed_data) {
            sent, err := net.send(t.socket, framed_data[total_sent:])
            if err != nil {
                return false
            }
            total_sent += sent
        ...
        sent, err := net.send_to(t.udp_socket, t.server_addr, data)
...
tcp_recv :: proc(t: ^Transport, buffer: []u8, timeout_ms: int) -> ([]u8, bool) {
    ...
    received, err := net.recv(t.socket, recv_buf[:])
...
udp_recv :: proc(t: ^Transport, buffer: []u8, timeout_ms: int) -> ([]u8, bool) {
    if timeout_ms == 0 {
        net.set_blocking(net.any_socket(t.udp_socket), false)
    } else {
        net.set_blocking(net.any_socket(t.udp_socket), true)
    }

    received, _, err := net.recv_from(t.udp_socket, buffer)
    if err != nil || received <= 0 {
        return nil, false
    }
